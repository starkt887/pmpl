    ...
    try {
      ...
      const updates = querySnapshot.docs.map(async (document) => {
        ...
        if (!data.stops || !Array.isArray(data.stops)) {
          console.warn(`Skipping route ${document.id}: stops field is missing or invalid`);
          return;
        }
        ...
        await updateDoc(doc(db, "routes", document.id), updatedData);
...
export { addRoutesToFirestore, addBusesToFirestore, addCitiesToFirestore,updateRoutesFormat };
